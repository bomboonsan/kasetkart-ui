...
export const profileAPI = {
  ...
  getMyProfileSidebar: () => {
    ...
    return api.get('/users/me?populate[profile][populate]=avatarUrl&publicationState=preview');
  },
...
export const dashboardAPI = {
  ...
  getPersonnelByAcademicType: async (departmentId = null) => {
    try {
      let params = 'populate=academic_type'
      if (departmentId) {
        params += `&filters[department][id][$eq]=${departmentId}`
      }
      
      const response = await api.get(`/profiles?${params}`)
      const profiles = response?.data || response || []
      
      // Group by academic type
      const grouped = {}
      profiles.forEach(profile => {
        const academicType = profile.academic_type?.name || 'Unknown'
        if (!grouped[academicType]) {
          grouped[academicType] = 0
        }
        grouped[academicType]++
      })
  ...
  getResearchStatsByTypes: async (departmentId = null) => {
    try {
      let params = 'populate[ic_types]=*&populate[impacts]=*&populate[sdgs]=*'
      if (departmentId) {
        params += `&filters[department][id][$eq]=${departmentId}`
      }
      
      const response = await api.get(`/project-researches?${params}`)
      const projects = response?.data || response || []
      
      // Get all types
      const [icTypesRes, impactsRes, sdgsRes] = await Promise.all([
        valueFromAPI.getIcTypes(),
        valueFromAPI.getImpacts(),
        valueFromAPI.getSDGs(),
      ])
      
      const icTypes = icTypesRes?.data || icTypesRes || []
      const impacts = impactsRes?.data || impactsRes || []
      const sdgs = sdgsRes?.data || sdgsRes || []
      
      // Count projects by each type
      const stats = {
        icTypes: icTypes.map(type => ({
          name: type.name,
          count: projects.filter(p => p.ic_types?.some(ic => ic.id === type.id)).length
        })),
        impacts: impacts.map(impact => ({
          name: impact.name,
          count: projects.filter(p => p.impacts?.some(i => i.id === impact.id)).length
        })),
        sdgs: sdgs.map(sdg => ({
          name: sdg.name,
          count: projects.filter(p => p.sdgs?.some(s => s.id === sdg.id)).length
        }))
      }
...
export const projectAPI = {
  ...
  getMyProjects: async () => {
    try {
      ...
      // Fallback: return all projects if filtering fails
      try {
