...
export const projectAPI = {
  ...
  getMyProjects: async () => {
    try {
      ...
      // For now, just get all projects to avoid complex populate issues
      // Later we can implement proper filtering
      const allProjects = await api.get('/project-researches')
      
      // Return all projects for now (can be filtered later when we understand the schema better)
      return allProjects
    } catch (error) {
  ...
  getProjectPartners: (projectId) => {
    return api.get(`/project-partners?populate=*&filters[project_research][documentId][$eq]=${projectId}`)
  },
