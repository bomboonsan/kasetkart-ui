...
export const profileAPI = {
  ...
  getMyProfileSidebar: () =>
    api.get(
      "/users/me?populate[profile][populate]=avatarUrl&populate[role]=*&publicationState=preview",
    ),
  // หมายเหตุ (ไทย): ดึงข้อมูลโปรไฟล์แบบเต็มสำหรับหน้าแก้ไข - แก้ไขให้ populate จาก User entity เท่านั้น
  getMyProfile: () =>
    api.get(
      "/users/me?populate[profile][populate]=avatarUrl&populate[organization]=*&populate[faculty]=*&populate[department]=*&populate[academic_type]=*&populate[participation_type]=*&populate[educations][populate]=education_level&publicationState=preview",
    ),
  // คอมเมนต์ (ไทย): ดึงข้อมูลผู้ใช้ตาม ID หรือ documentId สำหรับ Admin (แก้ไขให้รองรับทั้ง 2 รูปแบบ)
  getUserById: async (id) => {
    const populateQuery =
      "populate[profile][populate]=avatarUrl&populate[organization]=*&populate[faculty]=*&populate[department]=*&populate[academic_type]=*&populate[participation_type]=*&populate[educations][populate]=education_level&publicationState=preview";
  ...
  getProfile: (documentId) =>
    api.get(`/profiles/${documentId}?populate=*&publicationState=preview`),
  createProfile: (data) => api.post("/profiles", { data }),
  updateProfileData: (documentId, data) =>
    api.put(`/profiles/${documentId}`, { data }),
  findProfileByUserId: async (userId) => {
    if (!userId) return null;
    const response = await api.get(
      `/profiles?filters[user][id][$eq]=${userId}&publicationState=preview`,
    );
