...
export const worksAPI = {
  ...
  getMyConferences: async (params = {}) => {
    try {
      ...
      const allConferents = await api.get('{{baseUrl}}/work-conferences?populate=project_research.research_partners');
      const list = allConferents?.data || allConferents || []
      const filtered = Array.isArray(list) ? list.filter(conference => (conference.project_research?.research_partners || []).some(partner => partner.users_permissions_user?.id === userId)) : []
