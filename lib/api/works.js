...
export const worksAPI = {
  ...
  getMyBooks: async (params = {}) => {
    try {
      ...
      try { 
        const all = await api.get('/work-books?populate=*&status=draft')
        const userId = await getCurrentUserId()
  ...
  getBook: (id) => api.get(`/work-books/${getDocumentId(id)}?populate=*&status=draft`),
  ...
  getMyConferences: async (params = {}) => {
    try {
      const userId = await getCurrentUserId()
      const allConferences = await api.get('/work-conferences?populate=project_research.research_partners.users_permissions_user&status=draft')
      const myConferences = filterConferencesByProjectParticipation(allConferences, userId)
      return { data: myConferences }
    } catch (error) { 
      try { 
        const all = await api.get('/work-conferences?populate=*&status=draft')
        const userId = await getCurrentUserId()
        const filtered = filterItemsByUserId(all, userId)
        return { data: filtered }
      } catch (e) { 
  ...
  getConference: (id) => api.get(`/work-conferences/${getDocumentId(id)}?populate=*&status=draft`),
  ...
  getPublication: (id) => api.get(`/work-publications/${getDocumentId(id)}?populate=*&status=draft`),
