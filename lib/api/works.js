...
export const worksAPI = {
  ...
  getMyConferences: async (params = {}) => {
    try { const userRes = await api.get('/users/me'); const userId = userRes.data?.id||userRes.id; if(!userId) throw new Error('ไม่สามารถดึงข้อมูลผู้ใช้ได้'); const searchParams = new URLSearchParams(); searchParams.set('filters[users_permissions_user][id][$eq]', userId); for (const [k,v] of Object.entries(params||{})) searchParams.set(k,v); return await api.get(`/work-conferences?${searchParams.toString()}`) } catch(error){ try { const all = await api.get('/work-conferences?populate=*'); const list = all?.data||all||[]; const userRes = await api.get('/users/me'); const userId = userRes.data?.id||userRes.id; const filtered = Array.isArray(list)? list.filter(item=>{ const owners = item.users_permissions_user||item.authors||item.presenters||item.user||[]; return Array.isArray(owners)? owners.some(o=>o?.id===userId): owners?.id===userId }):[]; return { data: filtered } } catch(e){ throw error } }
  },
