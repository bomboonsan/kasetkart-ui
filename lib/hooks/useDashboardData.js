...
import { projectAPI, worksAPI, userAPI } from '@/lib/api'
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        const [projectsRes, worksRes, usersRes] = await Promise.all([
        ...
        const departmentPersonnelData = [
          { category: 'Admin', personnel: usersByRole.ADMIN || 0, percentage: ((usersByRole.ADMIN || 0) / totalUsers * 100).toFixed(1) },
          { category: 'User', personnel: usersByRole.USER || 0, percentage: ((usersByRole.USER || 0) / totalUsers * 100).toFixed(1) },
          { category: 'SuperAdmin', personnel: usersByRole.SUPERADMIN || 0, percentage: ((usersByRole.SUPERADMIN || 0) / totalUsers * 100).toFixed(1) }
        ]
